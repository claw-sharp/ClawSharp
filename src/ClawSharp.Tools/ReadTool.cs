...
internal sealed class ReadTool : BaseTool
{
    ...
    {
        if (!FileToolInputParser.TryParseRead(arguments, out var input, out _) || input is null)
        {
            return null;
        }

        if (!string.IsNullOrWhiteSpace(input.Pages))
        {
            return $"{input.FilePath} · pages {input.Pages}";
        }

        if (input.Offset > 1 || input.Limit is not null)
        {
            var startLine = input.Offset <= 1 ? 1 : input.Offset;
            return input.Limit is > 0
                ? $"{input.FilePath} · lines {startLine}-{startLine + input.Limit.Value - 1}"
                : $"{input.FilePath} · from line {startLine}";
        }
