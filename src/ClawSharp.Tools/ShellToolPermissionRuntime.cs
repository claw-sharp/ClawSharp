// TS origin: ./utils/permissions/permissions.ts, ./tools/BashTool/BashTool.tsx, ./tools/PowerShellTool/PowerShellTool.tsx
...
internal static class ShellToolPermissionRuntime
{
    ...
    {
        var cacheKey = CreateCacheKey(toolName, command, caseInsensitive);
        ...
        var promptDecision = await permissionPrompter.PromptAsync(
    ...
    private static string CreateCacheKey(string toolName, string command, bool caseInsensitive)
    {
        var normalizedCommand = caseInsensitive
